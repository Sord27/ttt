...
"""
//! \brief A script to check if foundation is configured or not
//! \version v1
...
"""
Output file columns:
| Query MAC | Order Number | Configured | Status |

----------------------------------------------------
    Field        Interpretation
----------------------------------------------------
      Y           If configured then yes
      N           If not configured the no
...
def debugging(debug, debugToFile):
    if debugToFile:  # log to file
        logging.basicConfig(level=logging.DEBUG,
                            ...
                            filename='foundation_config_check.log',
                            filemode='w')
...
def check_fnd_config(line):
    ...
    if fnd_available(MAC):
        ...
        value = getCdcKey(MAC, 'MFST')
        if value is not None:
            fnd_config = int(value[0:2], 16)
            if fnd_config & 0x40:
            ...
            print(MAC + "," + order_no + ",x,Error in getting MFST")
            fwrite.write(MAC + "\t" + order_no +
                         "\tx\tError in getting MFST" + '\n')
    else:
        print(MAC + "," + order_no + ",x,FND offline/not available")
        fwrite.write(MAC + "\t" + order_no +
                     "\tx\tFND offline/not available" + '\n')


def usage():
    print("python foundation_config_check.py <server_name> <input_file_name>")
...
def Main():
    ...
    if len(sys.argv) == 3:
        ...
        if os.path.isfile(input_filename):
            ...
            output_filename = "foundation_config_check_" + \
            ...
            results = pool.map(check_fnd_config, lines)
