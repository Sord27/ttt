...
"""
//! \brief A script to check foundation firmware version
//! \version v1
...
"""
Output file columns:
| Query MAC | MFVR | Status |

----------------------------------------------------
    Field        Interpretation
----------------------------------------------------
    Data          Returned value from MFVR bamio key
    None          Error with failure reason
----------------------------------------------------
"""
...
def debugging(debug, debugToFile):
    if debugToFile:  # log to file
        logging.basicConfig(level=logging.DEBUG,
                            ...
                            filename='foundation_fw_check.log',
                            filemode='w')
...
def check_fnd_fw(line):
    ...
    if fnd_available(MAC):
        # Get foundation firmware version
        value = getCdcKey(MAC, 'MFVR')
        if value is not None:
            print(MAC + "," + value + ",Success")
            fwrite.write(MAC + "\t" + value + "\tSuccess" + "\n")
        else:
            print(MAC + ",None,Error in getting MFVR")
            fwrite.write(MAC + "\tNone\tError in getting MFVR" + "\n")
    else:
        print(MAC + ",None,FND offline/not available")
        fwrite.write(MAC + "\tNone\tFND offline/not available" + "\n")


def usage():
    print("python foundation_fw_check.py <server_name> <input_file_name>")
...
def Main():
    ...
    if len(sys.argv) == 3:
        ...
        if os.path.isfile(input_filename):
            ...
            output_filename = "foundation_fw_check_" + sys.argv[1] + "_" + timestamp + ".txt"
            ...
            results = pool.map(check_fnd_fw, lines)
