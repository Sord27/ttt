...
local srcNodeText = ProtoField.string("sn.source_node.text", "Source Node Name", base.ASCII)
local srcNodeHex = ProtoField.uint8("sn.source_node.value", "Source Node Value", base.HEX)
...
local cmdIdText = ProtoField.string("sn.command_id.text", "Command Name", base.ASCII) 
local cmdIdHex = ProtoField.uint8("sn.command.value", "Command Value", base.HEX)
...
local packetLength = ProtoField.uint16("sn.packet_length", "Packet Length", base.DEC)

-- add categories to the protocol
ieee802154_sn_proto.fields = { srcNodeText, srcNodeHex, srcID, desID, nodeMatch, idMatch, cmdIdText, cmdIdHex, subcmdId, payloadLength, payload, checksum, packetLength, subCmdWithPayload }

-- returns correct key for a command
local function getCommand( command_map, pktSrcNode, pktcmdIdText )
	...
	for cmd_class, cmd_class_members in pairs(command_map) do
		for _, cmd_class_member in pairs(cmd_class_members) do
			...
			key = bit32.bor(bit32.lshift(cmd_class, 8), pktcmdIdText)
...
function ieee802154_sn_proto.dissector( tvbuffer, pinfo, tree )
	...
	local srcNodeTree = subtree:add("Source Node")
	local cmdIdTree = subtree:add("Command ID")

	srcNodeTree:add(srcNodeText, tvbuffer(2,1), device_names[pktSrcNode])
	srcNodeTree:add(srcNodeHex, tvbuffer(2,1), pktSrcNode)

	cmdIdTree:add(cmdIdText, tvbuffer(10,1), commands[key])
	cmdIdTree:add(cmdIdHex, tvbuffer(10,1), tvbuffer(10, 1):uint())

	subtree:add(srcID, tvbuffer(3,2), string.format("0x%04x", tvbuffer(3, 2):uint()))
	subtree:add(desID, tvbuffer(5,2), string.format("0x%04x", tvbuffer(5, 2):uint()))
