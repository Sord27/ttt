...
if device == 0:
    ...
    if ra_Right_Status == "true" or ra_Left_Status == "true":
        ...
        if ra_Right_Status == "true":
            ...
            yesterdayfind_ra_Right_activities = re.compile(
                (re.escape(yesterdayDate)) + '(\s)' + (timelimit) + '(:\d\d:\d\d\sR\sadj\s+\d+\-\>\s\d+)')
        ...
        if ra_Left_Status == "true":
            ...
            yesterdayfind_ra_Left_activities = re.compile(
                re.escape(yesterdayDate) + '(\s)' + timelimit + '(:\d\d:\d\d\sL\sadj\s+\d+\-\>\s\d+)')
