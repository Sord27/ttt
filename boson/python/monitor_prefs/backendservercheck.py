...
def getUsers(MAC, backendServer_URL, backendServer_UserName, backendServer_Password):
    ...
    userInfo.append(data["results"][0]["firstName"])
    userInfo.append(data["results"][0]["lastName"])
    userInfo.append(data["results"][0]["login"])
    #str("F:" + data["results"][0]["firstName"] + ",L:" + data["results"][0]["lastName"] + ",E:" + data["results"][0]["login"])
    return userInfo
...
def findBeServere(MAC):
    ...
    test = requests.get('https://circle2-be-api.circle.siq.sleepnumber.com/bam/rest/sn/v1/' + 'filteredUsers?filterByString=' + str(MAC), auth=HTTPBasicAuth(user, password), headers=headers)
    data = json.loads(test.text)
    logging.debug(json.dumps(data, indent=4, sort_keys=True))
    if len(data["results"]) != 0:
...
if __name__ == '__main__':
    ...
    if len(testcr) != 0:
        ...
        print("MAC could not be found in any backend server skip")
    '''
    for num in range(0, 14):
        g=string.ascii_uppercase[num]
        print (type(g))
        print("\n" + str(num) + "::" + string.ascii_uppercase[num])
