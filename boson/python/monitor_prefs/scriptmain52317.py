...
child = Get_PxsshObj(bosonServer, bosonUser, bosonPassword, bosonPort, bosonPrompt, bosonTimeout,
                     bosonSSHKEY)  # Get pexpect onject after connecting to Boson server
if child == 1:
...
for row in range(2, inputSheet.max_row + 1):  # Last value excluded so add 1
    ...
    if not ((MAC.startswith("CC04B4") or MAC.startswith("cc04b4") or MAC.startswith("64DBA0") or MAC.startswith(
            "64dba0")) and (len(MAC) == 12)):  # | "cc04b4" | "640000"):
